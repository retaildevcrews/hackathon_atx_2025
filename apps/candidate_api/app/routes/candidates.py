...
from app.models.candidate import Candidate, CandidateCreate, CandidateMaterial, CandidateMaterialList
from app.services import candidate_service, material_service
...
def upload_material(candidate_id: str, file: UploadFile = File(...)):
    ...
    if not c:
        raise HTTPException(status_code=404, detail="Candidate not found")
    try:
        return material_service.create_material(candidate_id, file)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
def list_materials(candidate_id: str):
    ...
    if not c:
        raise HTTPException(status_code=404, detail="Candidate not found")
    return material_service.list_materials(candidate_id)
...
def get_material(candidate_id: str, material_id: str):
    ...
    if not c:
        raise HTTPException(status_code=404, detail="Candidate not found")
    m = material_service.get_material(candidate_id, material_id)
    if not m:
...
def delete_material(candidate_id: str, material_id: str):
    ...
    if not c:
        raise HTTPException(status_code=404, detail="Candidate not found")
    deleted = material_service.delete_material(candidate_id, material_id)
    if not deleted:
        raise HTTPException(status_code=404, detail="Material not found")
    return {"success": True}
