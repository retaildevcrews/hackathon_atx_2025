"""Upload function package (Azure Functions entrypoint)."""
...
def main(req: func.HttpRequest) -> func.HttpResponse:  # type: ignore
    try:
        verify_shared_secret(req.headers.get(HEADER_NAME), settings.internal_upload_key_sha256)

        # Expect multipart. azure-functions does not parse multipart automatically.
        # We'll do a minimal parse using werkzeug if available, else simple fallback.
        content_type = req.headers.get("Content-Type", "")
        if "multipart/form-data" not in content_type:
            raise ValidationError("expected multipart/form-data")

        # Simple manual parse (works for single file + small fields); for robustness could add library.
        body = req.get_body()
        boundary_token = None
        ...
        for p in parts:
            ...
            headers_text = headers_blob.decode(errors='ignore').lower()
            if b"content-disposition" in headers_blob.lower():
                if b"name=\"file\"" in headers_blob.lower():
                    # extract filename
                ...
                elif b"name=\"contexttype\"" in headers_blob.lower():
                    context_type = data_blob.decode(errors='ignore').strip()
                elif b"name=\"contextid\"" in headers_blob.lower():
        ...
        blob_url = f"{settings.storage_account_url}/{settings.blob_container}/{blob_path}"

        # Stream upload in-memory chunking (already loaded here; future: parse stream incrementally)
